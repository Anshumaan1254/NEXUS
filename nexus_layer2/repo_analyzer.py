...
def get_gemini_client():
    try:
        ...
        # gemini-2.5-flash is a thinking model — needs long HTTP timeout (5 min)
        client = genai.Client(
            api_key=key,
...
def gemini_generate(client, prompt, temperature=0.2):
    ...
    for attempt in range(3):
        try:
            print(f"  [*] Gemini call attempt {attempt+1} (model={GEMINI_MODEL})...", flush=True)
            t0 = time.time()
            ...
            print(f"  [+] Gemini responded in {elapsed:.1f}s ({len(resp.text)} chars)", flush=True)
            return resp.text
        except Exception as e:
            elapsed = time.time() - t0
            print(f"  [!] Gemini attempt {attempt+1} failed after {elapsed:.1f}s: {e}", flush=True)
            if attempt < 2:
...
def get_active_contributors(repo_path: Path, days: int) -> str:
    ...
    result = subprocess.run(
        ["git", "--no-pager", "shortlog", "-sn", "--no-merges", f"--since={since_date}"],
        cwd=str(repo_path),
