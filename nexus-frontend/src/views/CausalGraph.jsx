...
export default function CausalGraph() {
  ...
  // Derive unique components for filter buttons — split multi-value components
  const componentList = useMemo(() => {
    const set = new Set()
    rawNodes.forEach(n => {
      if (n.component) {
        // Split "Frontend, Backend" into ["Frontend", "Backend"]
        n.component.split(/,\s*/).forEach(c => set.add(c.trim()))
      }
    })
    return ['ALL', ...Array.from(set).sort()]
  }, [rawNodes])

  // Filter nodes by component — use includes matching so "Backend" matches "Backend, Database"
  // When tracing, always include traced nodes so the full path is visible
  const nodes = useMemo(() => {
    if (filter === 'ALL') return rawNodes
    return rawNodes.filter(n => {
      // Always show traced nodes regardless of filter
      if (tracePath && tracePath.has(n.id)) return true
      // Partial match: "Backend" matches "Backend", "Backend, Database", "Storage, Backend"
      return n.component && n.component.split(/,\s*/).some(c => c.trim() === filter)
    })
  }, [rawNodes, filter, tracePath])
