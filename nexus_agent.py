    ...
    def scan(self) -> Dict[str, Any]:
        ...
        for root, dirs, files in os.walk(self.repo_path):
            ...
            for file in files:
                ...
                file_obj = Path(file)
                if file_obj.suffix in self.SOURCE_EXTS:
...
class Phase3DPRExtractor:
    ...
    def extract(self) -> List[DPR]:
        ...
        # If still need more DPRs, use regular commits
        if len(self.dprs) < self.config.min_dprs:
            for commit in self.git_result['commits']:
                if len(self.dprs) >= self.config.min_dprs:
                    break
                
                # Skip if already processed
                if commit.get('is_high_reasoning', False):
                    continue
                ...
                dpr = DPR(
                    ...
                    durability_reasoning="Regular commit without explicit reasoning",
                    causal_dependencies=[],
                    files_involved=commit['files'],
                    commit_refs=[commit['hash'][:8]],
                    involved_humans=[commit['author']],
                    assumption_decay_risk="low",
                    decay_risk_reasoning="Standard commit",
                    blast_radius_estimate="low",
                    blast_radius_reasoning="Regular change",
                    active_workarounds=[]
                )
